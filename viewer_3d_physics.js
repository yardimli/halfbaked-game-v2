...
function createCharacter(width, height, position, rotate) {
  ...
  let colShape = new Ammo.btBoxShape(new Ammo.btVector3(width, height / 2, 2));
...
//    gltf.geometry.center();
    ...
    root.traverse((obj) => {
      if (obj.isMesh) {
        ...
        // }
        // else {
        //   obj.position.y = 100; //position.y + ((Math.round(boxsize.y * 10000) / 10000) / 2)+100;				    //Position (y = up+, down-)
        // }
...
//    gltf.scene.scale(scale.x, scale.y, scale.z);
    ...
    if (rotate !== null) {
      ...
      if (load_from_scene) {
        ...
        quat1.setEulerZYX(0,rotate.y,0);
        transform.setRotation(quat1);
//        transform.setRotation(new Ammo.btQuaternion(0,Math.PI/2,0, Math.PI));
        // obj.rotation.x = rotate.x;
        // obj.rotation.y = rotate.y;
        // obj.rotation.z = rotate.z;
      }
      else {
        //      transform.setRotation(new Ammo.btQuaternion(THREE.Math.degToRad(rotate.x), THREE.Math.degToRad(rotate.y), THREE.Math.degToRad(rotate.z), 1));
        // obj.rotation.x = THREE.Math.degToRad(rotate.x);
        // obj.rotation.y = THREE.Math.degToRad(rotate.y);
        // obj.rotation.z = THREE.Math.degToRad(rotate.z);
      }
...
//    let colShape = new Ammo.btBoxShape(new Ammo.btVector3(boxsize.x - 18, boxsize.y - 14, boxsize.z - 18));
...
//  xObjectBody.setCollisionFlags( 2 );
    ...
    gltf.scene.userData.physicsBody = xObjectBody;
...
function move(location, destination, speed = playerSpeed) {
  ...
  var posY = transform.getOrigin().y();
...
//  PlayerBody.setWorldTransform(tbv31);
  ...
  if ((Math.floor(posX) <= Math.floor(newPosX) + 2.5 &&
    ...
    // tbv31.setValue(NewX,NewY,NewZ);
    // console.log(NewX);
    // location.setWorldTransform(tbv31);
